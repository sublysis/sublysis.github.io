# Laurent Schüller

**Cyber Security Analyst** | [Email](mailto:your-email@example.com) | [LinkedIn](https://linkedin.com/in/your-profile) | [GitHub](https://github.com/your-username) | [Phone](tel:+49123456789) | [Location](#)

---

[![trophy](https://github-profile-trophy.vercel.app/?username=sublysis&theme=onedark&title=LongTimeUser,Experience,Commits)](https://github.com/sublysis/)

---

## 📌 Professional Summary

Results-driven **Cyber Security Analyst** with expertise in **penetration testing, SIEM, compliance, and threat intelligence**. Passionate about identifying vulnerabilities, implementing security measures, and protecting organizational assets. Strong background in **risk assessment, incident response, and security architecture**.

---

## 💼 Work Experience

### **Cyber Security Analyst**

**[Company Name]** | [Month/Year] – [Month/Year or Present]

- Conducted **vulnerability assessments** and **penetration tests** to identify and mitigate security risks.
- Implemented and managed **SIEM solutions** (e.g., Splunk, ELK) for real-time threat detection.
- Developed and enforced **security policies** and **compliance frameworks** (e.g., ISO 27001, NIST).
- Collaborated with cross-functional teams to **respond to security incidents** and improve overall security posture.
- Automated security tasks using **Python, Bash, and PowerShell** scripts.

### **[Previous Position]**

**[Company Name]** | [Month/Year] – [Month/Year]

- [Responsibility 1]
- [Responsibility 2]
- [Responsibility 3]

---

## 🎓 Education

### **[Degree Name]**

**[Institution Name]** | [Month/Year] – [Month/Year]

- **Field of Study:** [Your Major or Focus]
- **Relevant Coursework:** [List relevant courses, e.g., Network Security, Cryptography, Ethical Hacking]

---

## 🛡️ Skills

### **Technical Skills**

- **Penetration Testing:** Metasploit, Burp Suite, Nessus, OpenVAS
- **SIEM & Logging:** Splunk, ELK Stack, Wazuh, Graylog
- **Network Security:** Firewalls (Cisco, Palo Alto), IDS/IPS (Snort, Suricata)
- **Compliance & Frameworks:** ISO 27001, NIST, GDPR, CISSP, CEH
- **Programming:** Python, Bash, PowerShell, SQL
- **Cloud Security:** AWS, Azure, GCP (IAM, Security Groups, Encryption)
- **Threat Intelligence:** MISP, AlienVault OTX, ThreatConnect
- **Incident Response:** Forensic Analysis, Malware Analysis, IR Plans

### **Soft Skills**

- Analytical Thinking
- Problem-Solving
- Team Collaboration
- Communication (Technical & Non-Technical)

---

## 🏆 Certifications

- **[Certification Name]** | [Issuing Organization] | [Year]
  - Example: **Certified Ethical Hacker (CEH)** | EC-Council | 2023
- **[Certification Name]** | [Issuing Organization] | [Year]
  - Example: **CISSP (Certified Information Systems Security Professional)** | (ISC)² | 2024

---

## 🚀 Projects

### **[Project Name]**

- **Description:** [Brief description of the project, e.g., "Developed a custom SIEM dashboard for real-time threat detection."]
- **Technologies:** [List technologies used, e.g., Python, Splunk, Docker]
- **GitHub:** [Link to Repository](#) | **Demo:** [Link to Demo](#)

### **[Project Name]**

- **Description:** [Brief description]
- **Technologies:** [List technologies]
- **GitHub:** [Link to Repository](#)

---

## 📜 Publications & Talks (Optional)

- **[Title of Publication/Talk]** | [Event/Conference Name] | [Year]
  - [Brief description or link](#)

---

## 🌍 Languages

- **German:** Native
- **English:** Fluent
- **[Other Language]:** [Proficiency Level]

---

<p><img align="center" src="https://github-readme-streak-stats.herokuapp.com/?user=sublysis&" alt="sublysis" /></p>

---

**© 2026 Laurent Schüller** | [GitHub](https://github.com/sublysis) | [LinkedIn](https://linkedin.com/in/sublysis)
